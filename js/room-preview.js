...
function drawFrontView(canvas, state) {
  ...
  // Body
  const bodyW = 0.15;
  ctx.fillRect(toX(personX - bodyW / 2), toY(personH - headR * 2 - 0.05), bodyW * scale, (personH - headR * 2 - 0.05) * scale);

  // Shoulders
  const shoulderW = 0.3;
  const shoulderTop = personH - headR * 2 - 0.05;
  ctx.fillRect(toX(personX - shoulderW / 2), toY(shoulderTop), shoulderW * scale, 0.08 * scale);

  // Head
  ctx.beginPath();
  ctx.arc(toX(personX), toY(personH - headR), headR * scale, 0, Math.PI * 2);
  ctx.fill();

  // Legs
  ctx.fillRect(toX(personX - 0.10), toY(0.75), 0.08 * scale, 0.75 * scale);
  ctx.fillRect(toX(personX + 0.02), toY(0.75), 0.08 * scale, 0.75 * scale);
